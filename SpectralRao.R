...
RAO <- NDVI
RAO[] <- SpectralRAO[[1]]
writeRaster(RAO, file=paste("RSDiversityES/RAO_", names(NDVI), ".tiff", sep=""), overwrite=TRUE)

#Shannon
Shannon <- NDVI
Shannon[] <- SpectralRAO[[2]]
writeRaster(Shannon, file=paste("RSDiversityES/Shannon_", names(NDVI), ".tiff", sep=""), overwrite=TRUE)
